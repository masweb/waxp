...
| Agente IA | LangChain + LangGraph + DeepSeek | `langchain/` |
...
├── langchain/              # Agente IA — LangChain + LangGraph
│   └── src/
│       ├── server.ts       # Servidor Express + streaming SSE
│       ├── graph.ts        # StateGraph LangGraph (agente ReAct)
│       ├── tools.ts        # Tools LangChain (CRUD de sites)
│       └── api.ts          # Cliente API autenticado para el backend CMS
└── README.md
```
...
Un agente conversacional construido con **LangChain + LangGraph + DeepSeek** que gestiona sites del CMS mediante lenguaje natural. Funciona como un servidor Express independiente con streaming SSE.

### Arquitectura

- **LLM**: `ChatDeepSeek` (`deepseek-chat`) con tools de LangChain vinculadas
- **Patrón de agente**: `StateGraph` de LangGraph con bucle ReAct — `llmCall` → condicional → `toolNode` → `llmCall` → … → `END`
- **Streaming**: Server-Sent Events (SSE) con streaming a nivel de token, eventos de tool calls y resultados de tools
- **Auth**: Cuenta dedicada del agente con auto-renovación JWT (caché de 1 hora)

### Tools disponibles

| Tool | Descripción |
|------|-------------|
| `search_sites` | Buscar sites por nombre o dominio (uso interno — resuelve nombres a IDs) |
| `create_site` | Crear site con nombre, dominio y locales (ISO 639-1). Genera header, footer y páginas por defecto |
| `update_site` | Actualizar nombre, dominio u opciones de un site por ID |
| `delete_site` | Eliminar site por ID (cascade: locales, páginas, bloques) |

### Comportamiento del sistema

- Pide todos los campos obligatorios antes de crear un site (nombre, dominio, locales)
- Resuelve automáticamente nombres/dominios a IDs vía `search_sites` — nunca pide el ID al usuario
- Responde en lenguaje natural, nunca expone JSON crudo ni detalles técnicos

### Ejecución

```bash
cd langchain
cp .env.example .env          # Configurar DEEPSEEK_API_KEY, API_BASE_URL, API_AUTH_EMAIL, API_AUTH_PASSWORD
pnpm install
pnpm dev                      # Servidor del agente en http://localhost:3001
```
...
| `API_BASE_URL` | URL del backend CMS | `http://localhost:8080` |
| `API_AUTH_EMAIL` | Email de autenticación del agente | — (requerido) |
| `API_AUTH_PASSWORD` | Password de autenticación del agente | — (requerido) |
| `PORT` | Puerto del servidor del agente | `3001` |
...
| Tecnología | Uso |
|------------|-----|
| LangChain | Definición de tools y abstracción del LLM |
| LangGraph | Agente StateGraph con bucle ReAct |
| DeepSeek | Proveedor LLM (`deepseek-chat`) |
| Express 5 | Servidor de streaming SSE |
| Zod | Validación de esquemas de tools |
...
- API key de DeepSeek (para el agente IA)
...
| **Agente IA** | `:3001` | Agente LangChain para CRUD de sites mediante lenguaje natural |
